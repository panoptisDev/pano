...
import (
	...
	"github.com/panoptisDev/pano/tests/contracts/block_hash"
	"github.com/panoptisDev/pano/tests/contracts/read_history_storage"
...
func TestBlockHash_EIP2935_DeployContract(t *testing.T) {
	...
	net := StartIntegrationTestNet(t,
		IntegrationTestNetOptions{
			...
			ModifyConfig: func(config *config.Config) {
				...
				// This has the benefit that the same tx will work in both ethereum and pano.
	...
	// network is running in Pano, no block hashes are stored in the history storage contract.
